...
tt = 11;
% tt = 9;
ttstr = 'yyyy-MM-dd''T''HH:mm:ss.SSS''Z';
...
DetcRange = Noise + 1.5.*STD;
meanDetcRange = mean(DetcRange);
...
RayScatter = Scatter(7,1);
EffScatter = Scatter(7,2);
...
% figure('name','E密度廓形')
% % Density = (NumberZ(KM75:KM125,3)+NumberZ(KM75:KM125,4))./2;
% Density = NumberZ(KM75:KM125,8);
...
% plot(AltitudeY,Density,'-k','linewidth',2);
% title('2023-07-21T15:15:53 Ca ion Density');
% xlabel('Altitude (km)');
% ylabel('Density (cm^{-3})');
% xlim([75 125]);
% 
...
% 
% figure('name','E光子数')
% % Density = (NZ(KM75:KM125,3)+NZ(KM75:KM125,4))./2;
% Density = NZ(KM75:KM125,8);
% Density(Density<0) = NaN;
% TimeX = datetime(TimeList, 'InputFormat', ttstr);
% TimeY = 1:size(TimeX,2);
% hours = hour(TimeX);
...
% AltitudeY = Altitude(KM75:KM125,:);
% plot(AltitudeY,Density,'-k','linewidth',2);
% title('2023-07-21T15:15:53 393 nm Counts');
% xlabel('Altitude (km)');
% ylabel('Counts');
% xlim([75 125]);
...
plot(AltitudeY,Density,'-k','linewidth',2);
title('2023-07-21T14:06:59 Ca ion Density');
xlabel('Altitude (km)');
ylabel('Density (cm^{-3})');
xlim([115 400]);

[max_value, max_index] = max(Density(13,:));
index = find(Density(13,:) > 100, 1);

figure('name','F光子数')
Density = NZ(KM150:KM400,:);
Density(Density<0) = NaN;
TimeX = datetime(TimeList, 'InputFormat', ttstr);
TimeY = 1:size(TimeX,2);
hours = hour(TimeX);
...
AltitudeY = Altitude(KM150:KM400,:);
plot(AltitudeY,Density,'-k','linewidth',2);
title('2023-07-21T14:06:59 393 nm Counts');
xlabel('Altitude (km)');
ylabel('Counts');
xlim([115 400]);
