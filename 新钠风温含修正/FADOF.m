...
for cc = 1:39
    for jt = 6:6
        for jj = 1:1001
                R_V_real_V = (VR.*Cnu(cc) - VL.*Cnu(cc)) ./ (V0);
                R_V_real_N = (NR.*Cnu(cc) - NL.*Cnu(cc)) ./ (N0);
                R_V_real_E = (ER.*Cnu(cc) - EL.*Cnu(cc)) ./ (E0);
                R_T_real_V = (VR.*Cnu(cc) + VL.*Cnu(cc)) ./ (2 * V0);
                R_T_real_N = (NR.*Cnu(cc) + NL.*Cnu(cc)) ./ (2 * N0);
                R_T_real_E = (ER.*Cnu(cc) + EL.*Cnu(cc)) ./ (2 * E0);
...
figure('name','北向风');
plot(Wind_N(:,Jnight),Altitude*(sqrt(3)/2),'-k','linewidth',2);
ylim([85 100]);
% xlim([-20 20]);
...
MapWind_N = Wind_N(KMx85:KMx100,Jnight);
...
figure('name','东向风');
plot(Wind_E(:,Jnight),Altitude*(sqrt(3)/2),'-k','linewidth',2);
ylim([85 100]);
% xlim([-20 20]);
...
MapTime = datenum(TimeX(:,Jnight));
MapAlt = Alt30(KMx85:KMx100,:);
MapWind_E = Wind_E(KMx85:KMx100,Jnight);
