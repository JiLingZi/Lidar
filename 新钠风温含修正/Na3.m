...
%% 读取光子数矩阵
ReadDate = '20231122';
fldstart = ['E:\RawData\ZWDATA\MOHEnew\Na\',ReadDate,'\'];
folder = [fldstart,'Na\'];
...
PSJ = 300;
XN = 8000/1500;
plot(Alt30,EF0(:,PSJ).*XN,'-b','linewidth',1.5)
xlim([70 110]); ylim([0 8000]);
xlabel('Altitude (km)'); ylabel('Density (cm^{－3})');
grid on;
title('Meteor Trail of Na Atom')
set(gca,'fontsize',15)

%% 单时刻光子数查验
PSJ = 846;
figure('name','ph','position',[100 50 800 800])
subplot(3,1,1)
plot(Altitude,VFR(:,PSJ),'-b','linewidth',1.5)
hold on; xlim([20 115]); ylim([0 2000])
plot([20 115],[500 500],'--r','linewidth',1.5)
ttstr = [char(string(TimeX(PSJ))),newline,' f_0']
title(ttstr)
set(gca,'fontsize',15)
yticks([0 500 1000 1500 2000]);
subplot(3,1,2)
plot(Altitude,VF0(:,PSJ),'-b','linewidth',1.5)
hold on; xlim([20 115]); ylim([0 2000]); ylabel('Counts')
plot([20 115],[500 500],'--r','linewidth',1.5)
set(gca,'fontsize',15)
yticks([0 500 1000 1500 2000]);
title('f_+')
subplot(3,1,3)
plot(Altitude,VFL(:,PSJ),'-b','linewidth',1.5)
hold on; xlim([20 115]); ylim([0 2000]); xlabel('Altitude (km)')
plot([20 115],[500 500],'--r','linewidth',1.5)
set(gca,'fontsize',15)
yticks([0 500 1000 1500 2000]);
title('f_-')
...
LdtT = datetime(2023, 11, 3, 19, 0, 0);
RdtT = datetime(2023, 11, 3, 25, 0, 0);
Ldt = datenum(LdtT);
Rdt = datenum(RdtT);
...
Temp_V(Temp_V>300) = NaN;
Temp_V(Temp_V<120) = NaN;
[Map, Line] = contourf(MapTime,Alt30(KMx75:KMx115,:),Temp_V(KMx75:KMx115,:),200);
...
Wind_N(Wind_N>150) = NaN;
Wind_N(Wind_N<-150) = NaN;
[Map, Line] = contourf(MapTime,Alt30(KMx75:KMx115,:),Wind_N(KMx75:KMx115,:),200);
set(Line,'LineColor','none')
ylabel(colorbar,'Meridional Wind (m/s)');
colormap(jet);
xlim([Ldt,Rdt]);
Ttk = 1/24;
x_ticks = Ldt:Ttk:Rdt;
...
Wind_E(Wind_E>150) = NaN;
Wind_E(Wind_E<-150) = NaN;
[Map, Line] = contourf(MapTime,Alt30(KMx75:KMx115,:),Wind_E(KMx75:KMx115,:),200);
set(Line,'LineColor','none')
ylabel(colorbar,'Zonal Wind (m/s)');
colormap(jet);
xlim([Ldt,Rdt]);
Ttk = 1/24;
x_ticks = Ldt:Ttk:Rdt;
...
figure('name','frequendy3sum')
plot(PhF0(:,Jnight),AltF,'-k','linewidth',1.5);
hold on;
plot(PhFR(:,Jnight),AltF,'-b','linewidth',1.5);
hold on;
plot(PhFL(:,Jnight),AltF,'-r','linewidth',1.5);
hold on;
ylim([75 115]);
% xlim([0 300]);
% ylim([100 110]);
...
xlabel('Photon Counts');
ylabel('Altitude (km)');
title('Na Zonal 20231102 12:02:08');
legend('f0','f+','f-');

figure('name','origin','position',[300 300 1200 350])
subplot(1,3,1)
plot(PhF0(:,Jnight),AltF,'-b','linewidth',1.5)
ylim([75 115]);
xlim([0 300]);
grid on;
xlabel('Photon Counts');
ylabel('Altitude (km)');
title('(a) f0 Na Zonal 20231105 20:53:08');
subplot(1,3,2)
plot(PhFR(:,Jnight),AltF,'-b','linewidth',1.5)
ylim([75 115]);
xlim([0 300]);
...
xlabel('Photon Counts');
ylabel('Altitude (km)');
title('(b) f+ Na Zonal 20231105 20:53:08');
subplot(1,3,3)
plot(PhFL(:,Jnight),AltF,'-b','linewidth',1.5)
ylim([75 115]);
xlim([0 300]);
grid on;
xlabel('Photon Counts');
ylabel('Altitude (km)');
title('(c) f- Na Zonal 20231105 20:53:08');
...
% hold on;
AltRay = 75000:1:115000;
[TemR,DenR] = atmosnrlmsise00(AltRay,53.3,122.7,2023,305,12);
Tray = TemR(:,2);
plot(Tray,AltRay./1000,'-r','linewidth',2)
hold on;
% plot(Saber_Temperture,Saber_Altitude,'-b','linewidth',2);
% xlim([180 260])
...
hold on;
scatter(272,84.2,50,'b','linewidth',2);
hold on;
e=errorbar(272,84.2,12,'horizontal');
e.Color = 'black';
e.Marker = 'none';
e.MarkerSize = 5;
e.CapSize = 5;
...
grid on;
legend('Nrlmsise-00','Mohe lidar','Meteor trail')

%% 模型温度
[TemRay,DenRay] = atmosnrlmsise00(105250,53.3,122.7,2023,306,12);
...
SER = sum(EFR(SL:SR,:),1);
SEL = sum(EFL(SL:SR,:),1);
SE0pre = sum(EF0(SL:SR,:),1);
SERpre = sum(EFR(SL:SR,:),1);
SELpre = sum(EFL(SL:SR,:),1);

% 计算RV
R_V_real_V = (SER - SEL) ./ (SE0);
R_V_real_N = (SER - SEL) ./ (SE0);
R_V_real_E = (SER - SEL) ./ (SE0);

% 计算RT
R_T_real_V = (SER + SEL) ./ (2 * SE0);
R_T_real_N = (SER + SEL) ./ (2 * SE0);
R_T_real_E = (SER + SEL) ./ (2 * SE0);


for jvt = 1:size(V0,2)
    Temp_Single(:,jvt) = FT(R_V_real_V(:,jvt),R_T_real_V(:,jvt),TruePxT,jvt);
end

T_Sig = Temp_Single(:,489)
...
RV_real = R_V_real_V(:,Jnight);
RT_real = R_T_real_V(:,Jnight);
Ph0 = SE0pre(:,Jnight);
PhR = SERpre(:,Jnight).*ErayVR(:,Jnight);
PhL = SELpre(:,Jnight).*ErayVL(:,Jnight);
...
subplot(1,3,1)
plot(PhF0(:,Jnight),AltF,'-b','linewidth',1)
ylim([25 115]);
grid on; hold on;
plot([BHD BHD],[25 115],'--r','linewidth',1)
xlabel('Photon Counts');
ylabel('Altitude (km)');
title('(a) f0');
set(gca,'fontsize',15)
subplot(1,3,2)
plot(PhFR(:,Jnight),AltF,'-b','linewidth',1)
ylim([25 115]);
grid on; hold on;
plot([BHD BHD],[25 115],'--r','linewidth',1)
xlabel('Photon Counts');
...
set(gca,'fontsize',15)
subplot(1,3,3)
plot(PhFL(:,Jnight),AltF,'-b','linewidth',1)
ylim([25 115]);
grid on; hold on;
plot([BHD BHD],[25 115],'--r','linewidth',1)
xlabel('Photon Counts');
