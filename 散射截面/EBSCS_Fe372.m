...
%激光常数
lambda_L = 372.0993e-9;%激光波长(m)
nu_L = c / lambda_L;%激光中心频率(Hz)

%专用常数
T = 200;%温度为200K条件下
M = 9.288e-26;%Fe原子绝对质量(kg)
fD1 = 0.0414;%Fe原子D1线的振子强度
fD2 = 0.0382;
IsoAbdn = 1;%Fe的同位素丰度
sigma_FWHM = 2.5e9;%激光测量线宽1.44GHz；半高全宽
sigma_L = sigma_FWHM / (2*sqrt(2*log(2)));%高斯线型的RMS宽度

%% 谱线参数赋值区
%跃迁谱线的中心频率偏差值
nu_0_1 = c/372.0993e-9;%Fe第1条跃迁谱线的中心频率
%Fe的超精细跃迁强度因子
f1 = 1;
...
%{
plot(nu-nu_L,sigma_abs_39,'.-b');%绘制K39的吸收截面
hold on
...
sprintf('Fe原子的有效后向散射截面为\n%e m^2/sr',sigma_K)
%{
plot(sigma_FWHM,sigma_K,'-','color',[0.37 0.15 0.07], 'Linewidth', 2)
set(gca,'FontSize',18);
