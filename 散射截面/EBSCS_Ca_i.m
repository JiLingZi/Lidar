...
A = (c/nu_0_1) * sqrt(k_B*T / (M*c^2))
S = (1 / (sqrt(2*pi)*A)) * (e^2 / (4*epsilon_0*m_e*c)) * fD1
...
%
plot(nu-nu_L,sigma_abs_1,'.-r');%绘制K39第1条线的吸收截面
hold on
...
plot([0, 0 ], [0 max_sigma_abs_1], '--k', 'linewidth', 2)
hold on;
plot([-1500e6, -1500e6 ], [0 0.4*max_sigma_abs_1], '--r', 'linewidth', 2)
hold on;
plot([1500e6, 1500e6 ], [0 0.4*max_sigma_abs_1], '--r', 'linewidth', 2)
hold on;
...
sprintf('Ca离子的有效后向散射截面为\n%e m^2/sr',sigma_K)
%{
plot(sigma_FWHM,sigma_K,'-g','Linewidth',2)
set(gca,'FontSize',18);
