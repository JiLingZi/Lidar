...
% 合并行，3行合并，96米
PhiSum = zeros(floor(8192/iNmu),num_files);
for i = 1:floor(8192/iNmu)
    PhiSum(i,:) = sum(DenPh(iNmu*(i-1)+1:iNmu*i,:),1);
end

% 镍原子原始分辨率为122.88米，因此未合并高度分辨率
% 合并列，108列合并，120分
PhjSum = zeros(floor(8192/iNmu),floor(num_files/jNum));
for j = 1:floor(num_files/jNum)
    PhjSum(:,j) = sum(PhiSum(:,jNum*(j-1)+1:jNum*j),2);
end

% 噪声130-150，计算光子数，高度
Altitude = height_num_origin(1:floor(8192/iNmu))*iNmu;
...
for jt = 1:ceil(num_files/24)
    ...
    fC = figure('name','Ni队列','position',[10 50 2400 1200]);
    subplot(4,6,1);
    ...
    figV = datestr(TimeX(:,jt),'yyyymmdd')+"-Ni-"+string(jt);
