...
for jt = 1:ceil(num_files/24)
    ...
    % 指定保存路径和文件名
    figPath = "F:\TrailIMG\Ca\"+ReadDate+"\";
...
%% 尾迹变化图
% TimeTrail = TimeX(:,1010:1013)';
% DenTrail = [31 81 35 11];
% AltTrail = [85.68 85.71 85.86 85.96];
% Profx = 1010;
% 
% yyaxis left
...
% TXcheck = CheckPh;
% for j = 1:size(CheckPh,2)
%     TXcheck(:,j) = (CheckPh(:,j)*1+j-1).*TXcal+Time(1);
% end
